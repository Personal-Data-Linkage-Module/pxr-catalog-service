...
/**
 * アクティベートAPIのバリデーションチェッククラス
 */
@Middleware({ type: 'before' })
export default class CatalogNamePostValidator implements ExpressMiddlewareInterface {
    async use (request: Request, response: Response, next: NextFunction) {
        ...
        // パラメータを取得
        const dto = await transformAndValidate(CatalogNamePostReqDto, request.body);
        // 配列はエラー
